...
/*******************************************************************************
* Function Name: Clock_Motor_Start
********************************************************************************
* Summary:
...
void Clock_Motor_Start(void) 
{
    ...
    Clock_Motor_CLKEN |= Clock_Motor_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: Clock_Motor_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Motor_StopBlock
********************************************************************************
* Summary:
...
{
    if (Clock_Motor_CLKEN & Clock_Motor_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = Clock_Motor_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Motor_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_Motor_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Motor_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Clock_Motor_StandbyPower
********************************************************************************
* Summary:
...
void Clock_Motor_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Clock_Motor_CLKSTBY &= ~Clock_Motor_CLKSTBY_MASK;
    }
    else
    {
        Clock_Motor_CLKSTBY |= Clock_Motor_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Clock_Motor_SetDividerRegister
********************************************************************************
* Summary:
...
void Clock_Motor_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    ...
    uint8 currSrc = Clock_Motor_GetSourceRegister();
    uint16 oldDivider = Clock_Motor_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Motor_CLKEN & Clock_Motor_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Clock_Motor_DIV_PTR, clkDivider);
                Clock_Motor_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: Clock_Motor_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(Clock_Motor_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Motor_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Motor_SetModeRegister(uint8 clkMode) 
{
    Clock_Motor_MOD_SRC |= clkMode & Clock_Motor_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Motor_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_Motor_ClearModeRegister(uint8 clkMode) 
{
    Clock_Motor_MOD_SRC &= ~clkMode | ~Clock_Motor_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_Motor_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Clock_Motor_MOD_SRC & Clock_Motor_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Motor_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Clock_Motor_GetDividerRegister();
    uint8 oldSrc = Clock_Motor_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_Motor_MOD_SRC |= CYCLK_SSS;
        Clock_Motor_MOD_SRC =
            (Clock_Motor_MOD_SRC & ~Clock_Motor_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Motor_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Clock_Motor_MOD_SRC & Clock_Motor_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Motor_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    Clock_Motor_PHASE = clkPhase & Clock_Motor_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Motor_GetPhase
********************************************************************************
* Summary:
...
{
    return Clock_Motor_PHASE & Clock_Motor_PHASE_MASK;
}
