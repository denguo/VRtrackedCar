...
void button_isr_Start(void)
{
    ...
    /* Set the priority. */
    button_isr_SetPriority(button_isr_INTC_PRIOR_NUMBER);
...
*   the component has been started use button_isr_StartEx instead.
*
*
* Parameters:
*   address: Address of the ISR to set in the interrupt vector table.
...
void button_isr_SetVector(cyisraddress address) 
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + button_isr__INTC_NUMBER] = address;
}
...
cyisraddress button_isr_GetVector(void) 
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + button_isr__INTC_NUMBER];
}
...
*   or button_isr_StartEx will override any effect this method would have had. 
*	This method should only be called after button_isr_Start or 
...
uint8 button_isr_GetState(void) 
{
    ...
    return (*button_isr_INTC_SET_EN & button_isr__INTC_MASK) ? 1:0;
}
